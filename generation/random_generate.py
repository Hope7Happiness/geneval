    ...
    for index, metadata in enumerate(metadatas):
        ...
        with torch.no_grad():
            ...
            for n in trange((opt.n_samples + batch_size - 1) // batch_size, desc="Sampling"):
                ...
                # ).images
                samples = torch.randn(min(batch_size, opt.n_samples - sample_count), 3, 256, 256)
                # get to PIL images
                samples = [Image.fromarray(((sample.clamp(-1, 1) + 1) / 2 * 255).cpu().numpy().transpose(1, 2, 0).astype(np.uint8)) for sample in samples]
